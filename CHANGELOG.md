...
* **Removed duplicate model dropdown in remote backend**: The `remote_model` selector in the top row of the interface (next to the server URL) has been removed. The active model is now exclusively selected via the `Whisper Model` dropdown inside `⚙️ Advanced Settings`, eliminating visual redundancy.
* **New connection status widget**: In place of the removed dropdown, a read-only `gr.Textbox` (`remote_status`) is now displayed, showing the real-time connection status with the remote server:
* `⚪ Waiting for connection...` — initial state at startup or when switching to the remote backend.
* `🟢 Connection successfully established ({model} - {device})` — after a successful connection test.
* `🔴 Connection error: {error}` — in case of connection failure.


* **Removed network blocking on startup**: UI initialization no longer triggers HTTP calls (`fetch_remote_models`, `check_server_health`) upon application launch. The app starts immediately without network delays, even if the remote backend is selected by default.
* **Removed Markdown status badge** (`remote_status_badge`): Replaced by the new `remote_status` Textbox. The message format has been updated from Markdown to plain text for compatibility with the widget.
* **Removed `_on_remote_model_change**`: The dedicated handler for the `remote_model` dropdown change event has been removed along with the `remote_model.change` listener. Server-side model switching logic is now entirely handled by `_on_whisper_model_change`.
* **Updated `_on_backend_change` handler**: Now yields a single output for the connection status (`remote_status`) instead of two separate outputs (`remote_status_badge` + `remote_model`). When switching to the remote backend, it sets the status to "Waiting for connection...".
* **Updated `_test_remote_conn` handler**: Reduced from 8 to 7 outputs by removing the redundant `remote_model` update; it now only populates `whisper_model` with models available on the server.
* **Updated `_on_whisper_model_change` handler**: Returns a single value (`remote_status`) instead of a tuple.
* **Updated `_on_compute_type_change` handler**: Output now targets `remote_status` instead of the legacy `remote_status_badge`.

### Localization

* Added 4 new keys to `settings/locales.yaml` (English + Italian):
* `remote_status_label` — status widget label (`Connection Status` / `Stato Connessione`).
* `remote_status_waiting` — waiting status text (`⚪ Waiting for connection...` / `⚪ In attesa di connessione...`).
