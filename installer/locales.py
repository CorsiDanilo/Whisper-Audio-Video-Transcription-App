...
STRINGS_EN: Dict[str, str] = {
    ...
    "config_existing_detected": "Existing settings detected in destination folder. Pre-filled below.",
...
STRINGS_IT: Dict[str, str] = {
    ...
    "config_existing_detected": "Impostazioni esistenti rilevate nella destinazione. Campi pre-compilati.",
...
def get_locale_strings(override_lang: Optional[str] = None) -> Dict[str, str]:
    """
    Return localizations dict for current OS or override language code ('en' / 'it').
    """
    if override_lang:
        code = override_lang.lower()
        if code in ("it", "italian"):
            return STRINGS_IT
        return STRINGS_EN

    try:
        sys_loc = locale.getdefaultlocale()[0]
        if sys_loc and sys_loc.lower().startswith("it"):
            return STRINGS_IT
    except Exception:
        pass
